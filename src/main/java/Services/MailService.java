...
public class MailService {
    ...
    public static void sendEmail(String to, String subject, String body, boolean html) {
        ...
        // ✅ CORRECTION 1 : Utiliser le bon Authenticator
        Session session = Session.getInstance(props,
                new Authenticator() {
                    @Override
                    protected jakarta.mail.PasswordAuthentication getPasswordAuthentication() {
                        // ✅ CORRECTION 2 : Utiliser le bon PasswordAuthentication (jakarta.mail)
                        return new jakarta.mail.PasswordAuthentication(FROM_EMAIL, APP_PASSWORD);
                    }
                });

        try {
            // ✅ CORRECTION 3 : Utiliser MimeMessage avec le bon import
            ...
            // ✅ CORRECTION 4 : Utiliser le bon Transport
            Transport.send(message);

            System.out.println("✅ Email envoyé avec succès à : " + to);

        } catch (MessagingException e) {
            LOGGER.log(Level.SEVERE, "❌ Erreur envoi mail : " + e.getMessage(), e);
